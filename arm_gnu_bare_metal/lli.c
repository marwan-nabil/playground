/* low level init function, valid for the AT91SAM7S MCU */
#include <stdint.h>
...
void low_level_init(void (*reset_addr)(), void (*return_addr)())
{
        ...
        static uint32_t const LDR_PC_PC = 0xE59FF000U;  // opcode of instruction ldr pc, [pc, ...]
        static uint32_t const MAGIC = 0xCAFEBABEU;      // to test if the remap operation has been done already
        AT91PS_PMC pPMC;
